...
const Demo = () => {
  ...
  }, [typedText, article.summary.length]);
  ...
  const handleSubmit = async (e) => {
    ...
    if(data?.summary) {
        const newArticle = { ...article, summary: data.summary, id: crypto.randomUUID() };
      ...
      <div className="flex flex-col w-full gap-2">
        ...
        <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
            {allArticles.map((item) => (
                <div key={item.id} onClick={() => setArticle(item)} className="link_card">
